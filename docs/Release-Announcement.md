...
The only data we keep on the network are songs. Playlists and accounts are local. Keys for logging into Autonomi are derived from imported (or generated by app) EVM private keys, encrypted with a password and stored locally, so we don't need to integrate MetaMask or any other wallet. For those, who feel uncomfortable with that, we advise having separate account just for JAMS and top it up from the main account. In case of accounts created by JAMS, private keys can be exported to other wallets.

JAMS was started when the Register datatype existed, so we used it, and since new datatypes were introduced, we emulate Register with a compound structure of Scratchpad, Transactions and Chunks. Our implementation differs from the high-level official one in that our register entries can contain unlimited amounts of data. This implementation is included in [Safe](https://github.com/JAMSplayer/safe) lib.
